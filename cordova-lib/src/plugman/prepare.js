...
module.exports = function handlePrepare(project_dir, platform, plugins_dir, www_dir) {
    ...
    // for windows phone and windows8 platforms we need to add all www resources to the .csproj file
    // first we need to remove them all to prevent duplicates
    var projFile;
    if(platform == 'wp8') {
        projFile = wp8.parseProjectFile(project_dir);
        ...
        projFile = windows8.parseProjectFile(project_dir);
